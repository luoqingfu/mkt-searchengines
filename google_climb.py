...
class GoogleClimb:
    ...
    def climb_google(self,key):
        ...
        httpRsp = requests.get("https://www.google.com.hk/search?q={}".format(key), headers=headersParameters)
        if httpRsp.status_code != 200:
            print("数据获取失败")
        else:
            soup = BeautifulSoup(httpRsp.text, "lxml")
            results = soup.select(".rc")
            print(results)
            # 用于保存提取的数据
            resultArr = []
    ...
    def write(self):
        ...
        for i in range(len(search_key)):
            result = self.climb_google(search_key[i])
